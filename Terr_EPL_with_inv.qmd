...
write.csv(tb2023, file = "outputs/sa_pa2023_inv2.csv")
...
epl2023 <- tb_tar %>%
  ...
  left_join(tb_vegrempa, by = "T_MAPCODE") %>%
  left_join(tb_invpa, by = "T_MAPCODE") %>%
    mutate(across(everything(), ~ replace_na(.x, 0))) %>%
    mutate(prp_pa_base = ext_vegpa/ext_veg) %>%
    # make targets km2
    mutate(ext_tar_sa = tar_sa/100 * ext_veg, 
           ext_tar_gbf = tar_gbf/100 * ext_veg) %>%
    #calc EPL  rounding proportions to 3 decimal places
  #BASE EPL using full extent  
  mutate(epl_base = ifelse(ext_vegpa >= ext_tar_sa, "WP", 
                             ifelse(ext_vegpa >= ext_tar_sa*0.5, "MP", 
                                    ifelse(ext_vegpa >= ext_tar_sa*0.05, "PP", "NP" )))) %>%
 #NATURAL EPL using natural extent (ignoring invasives)
   mutate(epl_nat = ifelse(ext_vegrempa >= ext_tar_sa, "WP", 
                             ifelse(ext_vegrempa >= ext_tar_sa*0.5, "MP", 
                                    ifelse(ext_vegrempa >= ext_tar_sa*0.05, "PP", "NP" )))) %>%
  ...
  mutate(ext_vegrempa_inv = ext_vegrempa - (ext_invpa*0.7)) %>%
  mutate(epl_nat_inv = ifelse(ext_vegrempa_inv >= ext_tar_sa, "WP", 
                             ifelse(ext_vegrempa_inv >= ext_tar_sa*0.5, "MP", 
                                    ifelse(ext_vegrempa_inv >= ext_tar_sa*0.05, "PP", "NP" ))))
 
#export the results of the adjusted EPL (epl_nat_exclinv is th enew data to compare to the standard EPL)
write.csv(epl2023, file = "outputs/results_df_EPL_2023_invasives2.csv") #export results 
```
...
epl2023inv_sum <- epl2023 %>%
  mutate(BIOME = str_replace_all(T_MAPCODE, 
                                  c("AT.*" = "Albany Thicket", 
                                    ...
                                    "SV.*" = "Savanna",
                                    "NOTVEG.*" = "NOTVEG"))) %>%
  group_by(BIOME,epl_nat_inv) %>%
  summarise(count = n(),.groups = 'drop') %>%
  pivot_wider(names_from = epl_nat_inv, 
...
write.csv(epl2023inv_sum, file = "outputs/results_df_EPL_2023_biome_invasives2.csv") #export results 
...
epl2023_sum <- epl2023 %>%
  mutate(BIOME = str_replace_all(T_MAPCODE, 
                                  c("AT.*" = "Albany Thicket", 
                                    ...
                                    "SV.*" = "Savanna",
                                    "NOTVEG.*" = "NOTVEG"))) %>%
  group_by(BIOME,epl_nat) %>%
  summarise(count = n(),.groups = 'drop') %>%
  pivot_wider(names_from = epl_nat, 
              values_from = count, values_fill = 0) %>%
  relocate(WP, .before = MP)%>%
  relocate(NP, .after = PP) %>%
   bind_rows(
...
write.csv(epl2023_sum, file = "outputs/results_df_EPL_2023_biome2.csv") #export results 

```

```{r}
data <- read_csv("C:/Rdata/EPL_terr/outputs/results_df_EPL_2023_biome_invasives.csv")
EPL <- data %>%
