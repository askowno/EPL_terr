...
#sa_pa_rall <- read_csv("outputs/sa_pa_rall.csv", show_col_types = FALSE) # iport test data for now but once the spatial aspects is run then use full version 
...
# Summary of natural vs not natural PA estate (LC=1 is natural) 
nat_pa_estate <- sa_pa_rall %>%
  filter(YEAR ==2023, !is.na(T_MAPCODE)) %>%
             group_by(LC_text, BIOME) %>%
  summarise(total_km2 = sum(km2),.groups = 'drop') %>%
  pivot_wider(names_from = LC_text, values_from = total_km2, values_fill = 0) %>%
   bind_rows(
      summarise(.,across(Nat:Notnat, \(x) sum(x , na.rm = TRUE)) %>%
            mutate(BIOME = "TOTAL")) ) %>%
  mutate(TOTAL = Nat + Notnat, prop_nat = Nat/TOTAL)
write.csv(nat_pa_estate, file = "outputs/results_df_natural_pa_estate_biome.csv") #export results
...
write.csv(epl, file = "outputs/results_df_EPL.csv") #export results 
