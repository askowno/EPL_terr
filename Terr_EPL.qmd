...
3\. Protected Areas vector feature geodatabse prepared by SANBI, built from Protected and Conserved Areas (PACA) database curated by South Africa's National Department of Forestry Fisheries and the Environment. It is a flattened spatial layer with no overlaps (PACA is not flattened and has overlapping features representing each legal declaration and declaration, and separate features for additioal assignments such as World Heritage Site status. This data also includes numerous defacto PAs, the information for which was provided by government Conservation authorities. A full description of the steps to generate the PA data can be found here...
...
#add PA data
pa <- vect("C:/Users/skownoa/Dropbox/NBAwork/NLC_change/NLC_Change.gdb", "SANBI_PA_2024Q2_AEA") #ensure it uses proj =  wgs84 aea cm 25 -24 -33 
```
...
write.csv(sa_pa_rall, file = "sa_pa_rall.csv") # a test vesion has been output sa_pa_rall_test based on aggregation to 400m pixels
```
...
#sa_pa_rall <- read_csv("sa_pa_rall.csv", show_col_types = FALSE) # iport test data for now but once the spatial aspects is run then use full version 
...
km2_sum_biome <- sa_pa_rall %>%   
  ...
  pivot_wider(names_from = YEAR, values_from = tot_km2, values_fill = 0) %>%
   bind_rows(
      summarise(.,across(`1990`:`2023`, sum, na.rm = TRUE)) %>%
            mutate(BIOME = "TOTAL"))  # Adjust this column if needed

#Summary of overall PA estate over time per biome
prop_sum_sa <- sa_pa_rall %>%   
...
targets <- sa_pa_rall %>%
  ...
  mutate(gbf_tar_km2 = 0.3 * type_tot_km2, nat_tar_km2 = T_CNSRV_TRGT/100 * type_tot_km2) 
...
epl <- targets %>% 
  ...
  mutate(prop_pa_2018 = `2018`/type_tot_km2) %>% #use backquotes to id the columns 
  mutate(prop_pa_2023 = `2023`/type_tot_km2) %>%
  mutate(across(where(is.numeric), ~ replace(., is.na(.), 0))) %>%
  mutate(epl_2023 = ifelse(`2023` >= nat_tar_km2, "WP", ifelse(`2023` >= nat_tar_km2/2, "MP", ifelse(`2023` >= nat_tar_km2/20, "PP", "NP")))) %>% 
  mutate(epl_2018 = ifelse(`2018` >= nat_tar_km2, "WP", ifelse(`2018` >= nat_tar_km2/2, "MP", ifelse(`2018` >= nat_tar_km2/20, "PP", "NP"))))
  
write.csv(epl, file = "results_df_EPL.csv") #test outputs to be changed to full output without "_test"
...
#export the graph
ggsave(
  filename = "pa_km2_year_plot.jpeg", # File name
...
#export the graph
ggsave(
  filename = "pa_km2_biome_plot.jpeg", # File name
...
#export the graph
ggsave(
  filename = "pa_prop_biome_year_plot.jpeg", # File name
  plot = plot,                            # Plot object
  device = "jpeg",                        # File format
